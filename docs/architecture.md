...
актуальный полный catalog через `MarketSource.discover_roles()`, случайно переставляет полный
набор ролей и строит двухфазный plan: сначала все resume-запросы, затем все vacancy-запросы.
