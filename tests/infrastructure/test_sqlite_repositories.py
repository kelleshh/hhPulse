...
async def test_claim_is_atomic_across_repository_instances(tmp_path) -> None:
    db_path = tmp_path / "hhpulse.sqlite3"
    database = SqliteDatabase(db_path)
    await database.initialize()
    jobs = SqliteAnalysisJobRepository(database)
    ...
    claims = await asyncio.gather(
        *(
            repository.claim_next_ready(
                planned.id,
                worker_id=f"worker-{index}",
                at=now,
            )
            for index, repository in enumerate((first, second) * 5)
        )
    )

    claimed_ids = [unit.id for unit in claims if unit is not None]
...
async def test_initialize_migrates_iteration_one_crawl_unit_schema(tmp_path) -> None:
    ...
    assert {"retry_at", "worker_id"}.issubset(columns)
